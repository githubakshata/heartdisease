...
def hello_world():
    if request.method=='POST':
        ...
        with open('patient_data.csv', 'a', newline='') as csvfile:
            ...
            # Write column names if the file doesn't exist
            if not file_exists:
                writer.writerow(['Name', 'Age', 'Anaemia', 'Creatinine Phosphokinase', 'Diabetes',
                                 'Ejection Fraction', 'High Blood Pressure', 'Platelets', 'Serum Creatinine',
                                 'Serum Sodium', 'Sex', 'Smoking', 'Max HR', 'Chest Pain Type',
                                 'Cholesterol', 'Family History', 'Medication', 'Probability'])

            writer.writerow([name, age, anaemia, creatinine_phosphokinase, diabetes, ejection_fraction,
                             high_blood_pressure, platelets, serum_creatinine, serum_sodium, sex, smoking,
                             maxhr, cp, cholesterol, fh, Medication, pv])
